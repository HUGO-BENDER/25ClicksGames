...
export class PanAndZoomDirective {
  // -- var DOM
  elementFrame: any;
  rectFrame: Rectangle;
  elementContent: any;
  ...
  ngAfterViewInit() {
    console.log('entramos al ngAfterViewInit');
    ...
    if (this.el.nativeElement) {
      ...
      const intervalIni = setInterval(
        (() => {
          if (
            this.elementContent !== undefined &&
            this.elementContent.clientWidth > 0
          ) {
  ...
  private onPan(event: MouseEvent) {
    if (this.frameOnPan) {
      ...
      if (this.valuesActual.cssScale === this.valuesMinFix.cssScale) {
        ...
        if (
          this.rectContent.width * this.valuesActual.cssScale >
          this.rectFrame.width
        ) {
        ...
        if (
          this.rectContent.heigth * this.valuesActual.cssScale >
          this.rectFrame.heigth
        ) {
  ...
  private calculateMinFix(): ScaleAndPoint {
    this.rectFrame = <Rectangle>{
      ...
      orientation:
        this.elementFrame.clientWidth > this.elementFrame.clientHeight
          ? 'landscape'
          : 'portrait',
    };
    this.rectContent = <Rectangle>{
      ...
      orientation:
        this.elementContent.clientWidth > this.elementContent.clientHeight
          ? 'landscape'
          : 'portrait',
    ...
    if (this.rectFrame.orientation !== this.rectContent.orientation) {
      if (this.rectFrame.orientation === 'landscape') {
        this.valuesMinFix.cssScale =
          this.rectFrame.heigth / this.rectContent.heigth;
        this.valuesMinFix.pointTopLeft = {
          x: Math.abs(
            (this.rectFrame.width -
              this.rectContent.width * this.valuesMinFix.cssScale) /
              2
          ),
          y: 0,
        };
        ...
        this.valuesMinFix.cssScale =
          this.rectFrame.width / this.rectContent.width;
        this.valuesMinFix.pointTopLeft = {
          x: 0,
          y: Math.abs(
            (this.rectFrame.heigth -
              this.rectContent.heigth * this.valuesMinFix.cssScale) /
              2
          ),
        };
      }
  ...
  private mouseWheelFunc(e: any) {
    ...
    const newScale = Math.max(
      this.valuesActual.cssScale + factor,
      this.valuesMinFix.cssScale
    );
    const xInTarget =
      e.pageX - this.elementFrame.offsetLeft - this.valuesActual.pointTopLeft.x;
    const yInTarget =
      e.pageY - this.elementFrame.offsetTop - this.valuesActual.pointTopLeft.y;
    if (newScale !== this.valuesMinFix.cssScale) {
      newX =
        this.valuesActual.pointTopLeft.x +
        (xInTarget - (xInTarget / oldScale) * newScale);
      newY =
        this.valuesActual.pointTopLeft.y +
        (yInTarget - (yInTarget / oldScale) * newScale);
    } else {
      const diffX =
        this.valuesActual.pointTopLeft.x - this.valuesMinFix.pointTopLeft.x;
      ...
      const diffY =
        this.valuesActual.pointTopLeft.y - this.valuesMinFix.pointTopLeft.y;
  ...
  private applyChanges(dataToApply: ScaleAndPoint) {
    let transformation =
      'translate(' +
      dataToApply.pointTopLeft.x +
      'px,' +
      dataToApply.pointTopLeft.y +
      'px)';
